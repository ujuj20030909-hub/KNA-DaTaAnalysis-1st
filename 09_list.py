...
# 리스트 안에 몇개의 값이 담겼는지 모르지만 마지막 요소를 뽑고싶다면?
...
# 슬랑이싱은 '있는 만큼만' 잘라주기 때문에 에러 발생하지 않음
...
print("===")  # remove()는 없는 값으 삭제하려고 할때 Error가 발생
# list1.remove("망고스틴") # ValueError
...
# .reverse()
# 값을 크기대로 정렬은 해주지 않음
# 뒤로 계속 쌓인 결과(최신)을 앞에서부터 보고싶을때 사용
