...
# 복습) 문자열 대소문자 비교
print("I" == "i") # False 대소문자를 항상 다르다고 봄

# break
# 반복을 그만 돌고 싶을 때
# 예 1) [1, 1, 3, 3, 2, 1, 1, 1]
# 위 리스트를 돌면서 10이상이 되면 중단하고 싶을때
...
while True: # 조건만 보면 무한반복하는 코드
  user_input = int(input("값을 입력하세요. 값의 누적이 15를 넘음녀 종료합니다."))
...
while True:
  # 변수 x는 반복을 돌때마다 재할당 되기 때문에 휘발되지만
  x = input("입력 (종료는 q를 입력하시오):")
...
# 첫 번째 입력값은 자동으로 최댓값이 됨(비교할 다른값이 없기 때문)
max_value = first
...
# v에는 방금 사용자가 입력한 값
# max_vlaue와 v의 값을 비교해 더 큰 값을 max_value에 재할당
if v > max_value:
  max_value = v
