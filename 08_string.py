...
print(
    "범위를 벗어난 슬라이싱", word[0:100]
)  # 범위를 벗어난 슬라이싱 PYTHON을 정상 출력
...
# SQE-00Q8이라는 설비의 SQE만 뽑아내기 (find랑 슬라이싱 사용)
...
# capitalize 는 문자열의 첫 글자만 대문자로 변환
...
# strip() 메서드에 인자로 들어가는 문자열은 완전히 동일하지 않아도 전체 삭제
str8 = "aaab 이렇게? cd"
...
# 지금 출려하는 결과는 " 이렇게? " 이렇게 나오고 있어
# 내가 생각했을 때 ==처럼 정확하게 "abcd" 순서가 아니면
# strip 이 안될 줄 알았는데 실행 결과를 보니 순서랑 상관없이
...
# ['에스프레소', '아메리카노', '카페라테']
# "띄어쓰기" 를 기준으로 나뉘어진 세 개의 문자열을 대괄호에 감싸서 출력
